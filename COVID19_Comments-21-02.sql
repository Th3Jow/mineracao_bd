...
--SELECT col_description(OID, 1) FROM pg_Class WHERE RelName='pacientes'
...
UNION
SELECT 'ExamLabs', A.AttName, Coli, Col_Description(C.OID, Coli) 
    FROM Pg_Class C, Pg_Attribute A, Generate_series(1, C.RelNAtts-1) Coli
    WHERE C.RelName='examlabs' AND A.AttRelID=C.OID AND A.AttNum=Coli
UNION
